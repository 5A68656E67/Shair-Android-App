...
public class TransactionService extends Service{
    ...
    public void onCreate() {
        ...
        this.myThread.start();
       // Toast.makeText(this, "transaction Service started", Toast.LENGTH_LONG).show();
    }
    ...
    private class MyThread extends Thread  {
        ...
        public void run() {
            ...
            while (true) {
                ...
                DefaultSocketClient client = new DefaultSocketClient(QueryType.TRANSACTION_NOTIFICATIONS,jsonObject);
                client.start();
                ...
                if (i == 0) {
                    ...
                    for (int i = 0; i < len; i++) {
                     ...
                     //   notification1.setTimeStamp(new SimpleDateFormat("HH:mm:ss  MM/dd/yyyy").format(new Date()));
                        TransactionActivity.transactions.add(notification1);
                    }
                } else {
                    JsonArray jsonArray = parser.parse(jsonStr).getAsJsonArray();
                    int len = jsonArray.size();
                    System.out.println("the number of received items:       " + len);
                    if (len != TransactionActivity.transactions.size() && len != 0) {
                        for (int i = TransactionActivity.transactions.size(); i < len; i++) {
                            JsonObject jsonObject1 = jsonArray.get(i).getAsJsonObject();
                            com.example.ethan.shairversion1application.entities.Notification notification1 = new com.example.ethan.shairversion1application.entities.Notification();
                            notification1.setItemName(jsonObject1.get("item_name").getAsString());
                            notification1.setNeederImg(jsonObject1.get("needer_img").isJsonNull() ? "https://s3.amazonaws.com/startupshair/itemimg/profile_image_default.png" : jsonObject1.get("needer_img").getAsString());
                            ...
                            notification1.setItemImg(jsonObject1.get("item_img").isJsonNull() ? "https://s3.amazonaws.com/startupshair/itemimg/no-image-thumb.png" : jsonObject1.get("item_img").getAsString());
       //                     notification1.setTimeStamp(new SimpleDateFormat("HH:mm:ss  MM/dd/yyyy").format(new Date()));
                            notification(notification1.getNeederName() + " got your item " + notification1.getItemName(), "1", "07/30/2015");
                            TransactionActivity.transactions.add(notification1);
                        }
                    }
